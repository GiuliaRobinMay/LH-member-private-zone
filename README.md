...
| Tab | What it does |
|---|---|
| **Chat with a Team Member** | A one-on-one chat window, like the support chat inside Mighty: navy band with the team's photo and the room name, a greeting from the team, and **Describe what you need…** with a red **Ask** button at the bottom. That opens a pop-up — ZIP code, state, and a big box to really describe what is going on — and sending it starts the conversation. The member's messages are warm tan bubbles on the right; the team answers on the left with their real profile photo (coloured initial where outside images are blocked), plus the links and file attachments they send along. Its ☰ is **My questions**: every conversation with its date and **Answered ✓ / No answer yet / New answer**; unread answers show as a red count on the tab. |
